...
function makeWorkerURL() {
  ...
  function analyzeCore(bitmap, sensitivity, controlPos, requireTwoLines, crop) {
    ...
    const valid = sel.peaks.filter(function (p) {
      return p.width <= maxWidth && p.z > 0.6;
    });
...
export default function LfaAnalyzer() {
  ...
  const advice = useMemo(
    () =>
      result?.diagnosis ? getRhinitisAdvice(result.diagnosis as Diagnosis) : null,
    [result?.diagnosis]
  );
