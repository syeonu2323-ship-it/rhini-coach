...
/** LFA QuickCheck v4.7 (Worker + Crop + Home/Finder 분리)
 * - 메인 스레드 프리즈 방지: Web Worker
 * - 대용량 다운스케일(≤1400px), 회전 탐색 최적화
 * - Crop 모드(사용자 지정 ROI)로 로고·여백 배제
 * - 처음 진입 시: 근처 약국·병원 찾기 화면
 * - 버튼 눌러서 키트 판독 화면으로 진입(카메라/사진 첨부)
 */
...
// -----------------------------
//   메인: 홈(근처 찾기) + 이미지 판독 UI
// -----------------------------
export default function LfaAnalyzer() {
  // 🔹 홈 / 판독 화면 전환
  const [view, setView] = useState<ViewMode>("finder");
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult({
          verdict: "Invalid",
          detail: "스트립을 찾지 못했습니다. (Crop 모드로 C/T 창만 지정해보세요)",
          confidence: "약함",
        });
      } else {
        setResult({
          verdict: "Invalid",
          detail: "처리 실패(알 수 없음). 다른 각도/밝기에서 재시도해주세요.",
          confidence: "약함",
        });
      }
    } catch (err: any) {
  ...
  const VerdictBadge = useMemo(() => {
    ...
    if (result.verdict === "Positive")
      return <span className={`${base} bg-red-100 text-red-800`}>✅ 양성</span>;
    if (result.verdict === "Negative")
      return <span className={`${base} bg-green-100 text-green-800`}>🟢 음성</span>;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-6">
      {view === "finder" ? (
        <>
          <h1 className="text-2xl sm:text-3xl font-semibold mb-1">🌬 Rhini-Coach</h1>
          <p className="text-sm text-gray-600 mb-4">
            먼저 내 위치 기준으로 근처 약국·병원을 확인하고, 필요하면 키트 판독으로 넘어가세요.
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-sm sm:text-base hover:bg-indigo-700"
            >
              📷 키트 판독하러 가기 (카메라/사진)
            </button>
            <p className="text-xs text-gray-500">
              * 모바일에서 열면 카메라로 바로 촬영해서 올릴 수 있어요.
            </p>
          </div>
        </>
      ) : (
          ...
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-sm text-gray-600">
                워커로 프리즈 방지 · 대용량 자동 축소 · Crop 모드로 C/T 창만 분석.
              </p>
            </div>
            <button
              onClick={() => setView("finder")}
              className="mt-1 px-3 py-1.5 rounded-lg border text-xs sm:text-sm text-gray-700 bg-white hover:bg-gray-50"
            >
              ← 근처 약국·병원 보기
            </button>
          </div>
          ...
          >
            <label className="w-full cursor-pointer">
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <div className="font-medium">카메라 촬영 / 사진 업로드 / 드래그</div>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                className="px-2 py-1 border rounded-md"
                value={controlPos}
                onChange={(e) => setControlPos(e.target.value as ControlPos)}
              >
                <option value="auto">자동</option>
          ...
          <div className="mt-4 p-4 rounded-2xl border bg-white">
            ...
            {mode === "crop" && (
              <div className="mt-2 text-xs text-amber-700">
                💡 ROI(초록박스) 안의 C/T 창만 분석합니다. 로고/글자/구멍은 제외하세요.
              </div>
            )}
          </div>
